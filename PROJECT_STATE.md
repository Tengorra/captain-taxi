...
- [ ] Apply Alembic migrations `002_icabbi_driver_fields` + `003_driver_full_icabbi_parity` on staging/prod DB before next iCabbi import
- [ ] Wire `login_password_hash` to a real auth flow if drivers actually need to log into the dashboard (currently parity-only)
...
| 8 | 2026-05-18 | Full iCabbi driver-record parity. **Migration `003_driver_full_icabbi_parity.py`** adds: driver auth (login_username/login_password_hash), personal extras (ethnicity, transporter, payment_card_last4/expiry), custom fields (pvg_disclosure, police_record, police_record_2), 7 attribute flags (pets, uniformed, topman, accept_discount/account/fixed_fares/cash_work), phone_assist, invoicing (invoice_footer, shift_reporting), payments/VAT (payment_on_day, distribution, apply_vat, vat_rate, balance, exclude_booking_fee, auto_post), bank (bank_payment_ref, use_sepa, bank_name, bank_account_name, sort_code, bank_account_number), breathalyser_enabled, driver fatigue (max_work_hours/min_rest_hours/exceed_job_pct/send_alert_pct). New tables: `driver_sites` (CTS/CTR assignments with primary flag), `driver_files` (HR-style uploads: police_disclosure, agreement, proof_of_address, photo_id, licence_photo/paper, pco_licence, insurance). **Vehicles** table extended with full iCabbi vehicle-dump columns (insurer, NCT/MOT, plate/hire/road_tax/council expiries, owner_driver, device/payment metadata, condition flags, seater counts 4-8, body types low_rider/estate/high_rider/sedan/minivan/suv); plate uniqueness relaxed to allow `001_Old` placeholder rows. New endpoints: `POST /vehicles/` (dedup by vehicle_ref) + `POST/GET/DELETE /drivers/{id}/files/...` (multipart upload to `DRIVER_FILE_ROOT`, defaults to `/data/driver_files`). Dashboard Drivers `ADD NEW DRIVER` rebuilt as a 5-step wizard (Personal → Licensing → Payments/VAT → Documents → Sites) with all new fields wired; files queue client-side and upload after the driver is created. New **IMPORT VEHICLE CSV** button (sky-blue, next to the iCabbi driver importer) parses the iCabbi vehicle dump (MM/DD/YYYY HH:mm date format, YES/-/blank toggles) and links each vehicle to its owning driver via `vehicle_ref`. **Follow-up in same session:** `DriverProfile.tsx` rebuilt to surface every new field — Details/Personal extended with Login/Password/Transporter/Ethnicity, Licensing extended with PVG, full Attributes (all 7 toggles), new sections for Custom Fields/Breathalyser, Payments+VAT, Bank, Driver Fatigue, Device+Phone Assist; Documents grid now uses the new `/drivers/{id}/files` endpoints (real upload, download, delete, with separate slots for licence photo + paper + PCO licence); Site Assignment table edits `siteSel` and persists via PATCH. Admin `PATCH /drivers/{id}` widened to accept the full DriverCreate field set with column-name aliases + site-replace logic. New **Vehicles** page (`/vehicles`) with iCabbi-style REF/PLATE/MAKE search + paginated table showing expiry dates in red when overdue, plus Truck-icon nav item in the Sidebar. **Backend deployability:** `admin/Dockerfile` now runs `alembic upgrade head` before uvicorn and honors Railway's `$PORT`; `railway.json` repointed at the admin service (was orchestrator, which the dashboard doesn't call). **Vehicle edit page:** new `GET /vehicles/{id}` + `PATCH /vehicles/{id}` + `DELETE /vehicles/{id}` endpoints; new `/vehicles/:id` route with `VehicleProfile.tsx` editing every iCabbi vehicle-dump field (identity, make/model, insurance & compliance expiries, device & payments, body type, seater capacity); Vehicles list rows are now clickable with an EDIT button. |
