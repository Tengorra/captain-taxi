...
**Last updated:** 2026-05-18 (session 8c)
...
- [x] iCabbi sync framework: `admin/services/icabbi_client.py` + `icabbi_sync.py` (per-entity upsert by `icabbi_ref`, stamps `last_synced_at`), `GET/POST /api/icabbi/{status,sync}`, nightly scheduler @ 3 AM. Off by default until `ICABBI_BASE_URL` + `ICABBI_API_KEY` are set.
- [x] Wire blacklist check into customer agent (customer/services/blacklist.py + create_booking gate in conversation.py — fail-open if admin unreachable)
- [x] Wire blacklist check into dispatch service (`dispatch/services/blacklist.py` + 403 in `POST /dispatch/trip` if blocked)
- [x] Items → Receipts pipeline: POST /api/receipts/from-trip snapshots Trip + listed item codes, applies 5% GST to taxable lines, refuses duplicate-per-trip
- [x] Receipt PDFs: `admin/services/pdf_service.py:generate_receipt_pdf` saves to `RECEIPTS_DIR`; `GET /api/receipts/{id}/pdf` streams (regenerates on miss)
- [x] Owner-statement auto-generation: admin/services/owner_statements_service.py rolls up completed trips by driver.vehicle_ref + commission from Settings. POST /api/owner-statements/generate (manual) + monthly scheduler job (1st @ 4am)
- [x] Owner-statement PDFs: `generate_owner_statement_pdf` saves to `STATEMENTS_DIR`; `GET /api/owner-statements/{id}/pdf` streams
- [x] Driver mandatory-field rules moved server-side: Settings key `driver_mandatory_fields` (JSON array); Drivers.tsx now validates against it instead of hard-coded names
- [x] Settings.tsx UI editor: toggle chips for driver_mandatory_fields + iCabbi sync status panel with "Run Sync Now" button
- [x] Bug fix: `admin/scheduler.py:_expire_escalations` — was importing a nonexistent `EscalationStatus` enum and referencing an `expires_at` column that doesn't exist on the model. Refactored to use string status `"open"/"expired"` and age-based expiry via the `escalation_timeout_hours` setting.
...
| 8c | 2026-05-18 | Finished the remaining no-creds work: (a) iCabbi sync framework — `admin/services/icabbi_client.py` (env-driven, off when not configured), `icabbi_sync.py` (per-entity upsert by `icabbi_ref`, stamps `last_synced_at`), `/api/icabbi/{status,sync}` endpoints, nightly scheduler @ 3am; (b) blacklist gate also in dispatch service (`dispatch/services/blacklist.py`, 403 in `POST /dispatch/trip`); (c) PDF generation for receipts + owner statements (ReportLab, branded, GET /api/receipts/{id}/pdf and /owner-statements/{id}/pdf, regenerate-on-miss); (d) Settings.tsx editor — toggle-chip UI for `driver_mandatory_fields` + iCabbi sync status panel with "Run Sync Now"; (e) bug fix: `_expire_escalations` was importing a nonexistent enum and a missing column — rewritten to use string status + age-based expiry from `escalation_timeout_hours`. |
| 8b | 2026-05-18 | Wired the no-creds follow-ups from session 8: (a) blacklist gate in customer agent (`customer/services/blacklist.py` → admin /api/blacklist/check, fail-open); (b) Items→Receipts pipeline (`POST /api/receipts/from-trip`, 5% GST on taxable lines, dedupe per trip); (c) owner-statement roll-up service + monthly scheduler job (`admin/services/owner_statements_service.py`, cron 1st @ 4am) + `POST /api/owner-statements/generate` + "Generate last month" dashboard button; (d) Driver mandatory-fields moved to Settings.driver_mandatory_fields (JSON array), Drivers.tsx now validates against it. |
